...
"""prints/displays the media info for all active media sessions on the system."""
...
async def trans_sesh(sesh):
    ...
    #Creates a dictionary of media info properties from a MediaProperties object.
    transed_sesh = {
        'title': getattr(props, 'title'),
        'artist': getattr(props, 'artist'),
        'album_title': getattr(props, 'album_title'),
        'album_artist': getattr(props, 'album_artist'),
        # Converts the genres property to a Python list.
        'genres': list(getattr(props, 'genres')),
        # Converts the thumbnail property to a PIL image.
        'thumbnail': await ref_to_img(getattr(props, 'thumbnail')),
        'track_number': getattr(props, 'track_number'),
        'album_track_count': getattr(props, 'album_track_count'),
        'playback_type': getattr(props, 'playback_type'),
        'subtitle': getattr(props, 'subtitle')
    }
    return transed_sesh
